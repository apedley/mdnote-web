...
describe('Pipe: TruncateEllipsisPipe', () => {
  ...
  beforeEach(() => {
    pipe = new TruncateEllipsisPipe();
  });


  it('should return the string if it\'s length is less than 200', () => {
    expect(pipe.transform('string')).toEqual('string');
  });

  it('should return up to 200 - 3 characters followed by an ellipsis', () => {
    expect(pipe.transform(longStr)).toEqual(`${longStr.substr(0, 197)}...`);
  });

  it('should return only 20 - 3 characters followed by an ellipsis', () => {
    expect(pipe.transform(longStr, 20)).toEqual(`${longStr.substr(0, 17)}...`);
  });
});
